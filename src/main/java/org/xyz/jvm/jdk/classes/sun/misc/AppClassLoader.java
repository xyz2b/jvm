...
import org.xyz.jvm.hotspot.src.share.vm.classfile.ClassFileParser;
...
public class AppClassLoader {
    ...
    // 缓存该类加载器加载的所有类
    private static Map<String, Handle> appClassLoaderData = new HashMap<>();

    // main函数所在类在此保存一份引用，方便快速定位
    private static Handle mainKlass = null;

    public static Handle getMainKlass() {
        return mainKlass;
    }

    public static void setMainKlass(Handle mainKlass) {
        AppClassLoader.mainKlass = mainKlass;
    }
    ...
    public static boolean isLoadedKlass(String className) {
        return appClassLoaderData.containsKey(className);
    }
     ...
     * @param className 需要查找的类全限定名
     * */
    public static Handle findLoadedKlass(String className) {
        return appClassLoaderData.get(className);
    }
     ...
     * */
    public static Handle saveLoadedKlass(String className, Handle klass) {
        return appClassLoaderData.put(className, klass);
    }
     ...
     * */
    public static Handle loadMainClass(String className) {
        if (mainKlass != null) {
            return mainKlass;
        }
     ...
     * @return 加载完成后生成的Klass模型InstanceKlass
     * */
    public static Handle loadKlass(String className) {
     ...
     * @return 加载完成后生成的Klass模型InstanceKlass
     * */
    public static Handle loadKlass(String className, boolean resolve) {
        // 查询缓存是否已经加载过了，如果是已经加载过的类直接返回
        Handle klass = findLoadedKlass(className);
        if (klass != null) {
            return klass;
        }
